...
class KinMS:
    ...
    def __init__(self, xs, ys, vs, cellSize, dv, beamSize, 
                 ...
                 cleanOut=False,  huge_beam=False, verbose=False,multiple_line_lams=False,spectral_resolution=0.0):
        """       
        :param xs:
        ...
        if np.any(multiple_line_lams):
            ...
            self.multiple_line_vels=doppler_opt_lam(multiple_line_lams[0],multiple_line_lams) # work out shifts from first line (reference)
        else:
            self.multiple_line_vels=np.zeros(1)
    ...
    def model_cube(self,inc, posAng, gasSigma=0, diskThick=0, flux_clouds=None, 
        ...
        if np.any(multiple_line_relflux):
            ...
            self.multiple_line_relflux=np.ones(1)
        ...
        for dvshift,fboost in zip(self.multiple_line_vels,self.multiple_line_relflux):
